"""
mode_intraday_long.py — Intraday long/buy screener.
...
# ── CORE LOGIC: buy-setup scoring (opposite of the short screener) ─────
def score_long(snap, params):
    try:
        ...
        dist_from_low = ((current_price - low_price) / low_price) * 100 if low_price else 0
        ...
        if dist_from_low < 1: score += 20
        elif dist_from_low < 2: score += 10
        ...
        return {
            "price": current_price, "open": open_price, "high": snap["day_high"], "low": low_price,
...
# ── UI ───────────────────────────────────────────────────────────────────
def render() -> None:
    st.markdown('<p class="main-header">📈 Intraday Long (Buy) Screener</p>', unsafe_allow_html=True)
    st.markdown("*Scan for stocks showing upward momentum for intraday buying*")
    ...
    with st.sidebar.expander("Advanced Thresholds"):
        price_change_threshold = st.slider("Price Change (%)", -1.0, 5.0, 0.0, 0.5, key=sskey(MODE_KEY, "price_chg_th"))
        momentum_threshold = st.slider("Momentum (%)", 0.0, 5.0, 0.5, 0.1, key=sskey(MODE_KEY, "momentum_th"))
        dist_from_low_threshold = st.slider("Dist from Low (%)", 0.5, 5.0, 2.0, 0.5, key=sskey(MODE_KEY, "dist_low_th"))
        volume_ratio_threshold = st.slider("Volume Ratio", 1.0, 3.0, 1.2, 0.1, key=sskey(MODE_KEY, "vol_ratio_th"))
        trend_threshold = st.slider("5-Day Trend (%)", 0.0, 10.0, 2.0, 0.5, key=sskey(MODE_KEY, "trend_th"))
    ...
    def fetch_and_analyze(rec):
        ...
        if snap is None:
            return "failed", None
        analysis = score_long(snap, params)
        if analysis is None:
    ...
    do_scan, resume_scan, checkpoint, _sig = sc.render_scan_trigger(
        MODE_KEY, stocks_to_scan, f"🔍 SCAN {len(stocks_to_scan)} STOCKS FOR BUY SETUPS")
    ...
    with st.expander("📚 How to Use"):
        ...
        with h1:
            ...
            **Best Scan Times:** 10:00–11:30 AM (post-opening) · 1:30–2:30 PM (post-lunch dip recovery)
            """)
        with h2:
            st.markdown("""
            **Signal Strength:** 🟢 STRONG (≥ Strong Signal Score) · 🟡 MODERATE (50+)

            **Risk Management:** Stop loss 0.5–1% below entry · Position size 1–2% of capital · Exit before 3:15 PM

            **Buy Conditions Checked:** price rising from open · near day low (bounce zone) · 5-day uptrend ·
            positive momentum · high volume · RSI oversold · good ATR volatility
            """)
...
def _render_results() -> None:
    ...
    df = pd.DataFrame([{
        ...
        "Dist from Low (%)": r["dist_from_low"], "5D Trend (%)": r["recent_trend"],
    ...
    styled = df.style.map(color_signal, subset=["Signal"]).map(color_change, subset=["Change %", "5D Trend (%)"]).format({
        ...
        "Dist from Low (%)": "{:.2f}%", "5D Trend (%)": "{:+.2f}%", "RSI": "{:.1f}", "ATR %": "{:.2f}%",
    })
